...
void captfils(int sig) {
	...
	/*
	la fonction rectvert() dans la routine captfils fait crasher la fenetre graphique si un control-C est envoyé au père 
	pour régler ce problème on modifie la valeur d'un booleen qui permettra d'activer la fonction directement dans le main()
	après l appel de rectvert on remet ce booleen à 0 pour éviter de maintenir le rectangle vert 
	La fonction rectvert() dans la routine captfils fonctionne si l'on envoie le signal via raise() mais pas avec le control-C dans le terminal
	La solution que nous avons choisi fonctionne dans le programme demandé cependant si nous avions comme objectif de définir 
	la fonction il aurait fallu que nous corrigions le problème de crash 
	Ici n'ayant pas développer le fichier sigx.c nous n'avons pas réussi à trouver l'erreur liée au crash
